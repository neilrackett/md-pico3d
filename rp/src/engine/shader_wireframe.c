...
static void render_line(color_t *fb, int32_t x1, int32_t y1, int32_t x2, int32_t y2) {
    if (x1 >= SCREEN_WIDTH || x1 < 0 || x2 >= SCREEN_WIDTH || x2 < 0 ||
        y1 >= SCREEN_HEIGHT || y1 < 0 || y2 >= SCREEN_HEIGHT || y2 < 0) {
        return;
    }
