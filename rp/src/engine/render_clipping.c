...
/* Produces a single clipped triangle when two vertices are behind the camera */
void clip_single_triangle(uint8_t vertex_in_screen, int32_t mat_vp[4][4],
    ...
    /* Resort incoming parameters depending on which vertex is still visible */
...
void clip_extra_triangle(uint8_t vertex_out_screen, int32_t mat_vp[4][4],
    struct triangle_32 *input_triangle, struct triangle_32 *output_triangle,
    struct triangle_32 *extra, int32_t w1, int32_t w2, int32_t w3) {

    struct triangle_32 in;

    /* Resort incoming parameters depending on which vertex is out of screen */
    ...
    /* Process first triangle: create a shortened version */
    int32_t t1 = -(clip_tri.vertex3.z - z_near) / ((clip_tri.vertex1.z - clip_tri.vertex3.z) / FIXED_POINT_FACTOR);
    ...
    /* Keep the edge between V2 and V3 for first triangle */
    ...
    /* For the second extra triangle, reuse V1 and V2, create new V3 near V2 */
