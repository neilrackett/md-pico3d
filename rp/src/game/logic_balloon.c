...
void logic_zombies() {
    ...
    for (int i = 0; i < MAX_ZOMBIES; i++) {
        if (zombie_list[i].status == -1 && player_kills <= 50) {
            ...
            int32_t chunk_x = rand_range(1, 10);
            int32_t chunk_y = rand_range(6, 10);

            if (chunk_physics[chunk_x][chunk_y] == 2) {
                zombie_list[i].status = 1;
                zombie_list[i].direction = rand_range(0, 3);
                ...
                uint8_t r = rand_range(0, 15);
                uint8_t g = rand_range(0, 15);
                uint8_t b = rand_range(0, 15);
    ...
    for (int i = 0; i < MAX_ZOMBIES; i++) {
        ...
        if (zombie_list[i].status == 1) {
            ...
            int32_t new_direction = rand_range(0, 250);
            if (new_direction == 0) {
                zombie_list[i].direction = rand_range(0, 3);
            }
