...
# Determine the file size — stcmd stat emits a prefix line, so extract the last token
filesize=$(ST_WORKING_FOLDER=$working_folder stcmd stat -c %s "$filename" 2>/dev/null | awk '{print $NF}')

# Check if the file is larger than 64Kbytes
if [ -n "$filesize" ] && [ "$filesize" -gt "$targetsize" ] 2>/dev/null; then
